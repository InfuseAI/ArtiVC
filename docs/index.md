...
Please see [the command reference](./commands/art.md)
...
- **Object storage or NFS:**  We can create folders or prefixes to add version semantics. It is straightforward to store different versions of data. But it lacks commit message, metadata, and history tracking. And we cannot know which one is the latest version.
...
- **Git LFS**: [Git LFS](https://git-lfs.github.com/) is an open-source Git extension for versioning large files.
    - If you want to leverage cloud object storage advantage, you need to install a proxy to store large files to object storage.
    - Git LFS does not seem suitable for machine learning workflows due to the lack of flexibility in configuring the cloud storage behind it.
    (ref: [https://mti-lab.github.io/blog/2021/03/03/dvc.html](https://mti-lab.github.io/blog/2021/03/03/dvc.html))
- **DVC**: [DVC](https://dvc.org/) is built to make ML models shareable and reproducible. It is designed to handle large files, data sets, machine learning models, and metrics as well as code.
    - it needs an additional DVC system and a git service to achieve data versioning.
- **LakeFS**: [LakeFS](https://lakefs.io/) provides a multi-server solution to make s3 as git-like repositories
    - the [architecture](https://docs.lakefs.io/understand/architecture.html) is much heavier than Art due to an extra database for metadata storage and S3 configuration. (e.g., S3 gateway)